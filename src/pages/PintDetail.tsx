...
const PintDetail = () => {
  ...
  return (
    <div className="max-w-md mx-auto pb-32 text-cream">
      {/* Full-bleed photo */}
      <section className="relative w-full aspect-[4/5]">
        ...
        <button
          onClick={() => navigate(-1)}
          className="absolute top-12 left-5 p-2.5 bg-black/40 backdrop-blur-md rounded-full text-white active:scale-90 transition-transform border border-white/10"
        >
        ...
        <div className="absolute bottom-6 right-5 bg-gold text-stout px-5 py-2.5 rounded-2xl shadow-xl">
          <span className="font-black text-2xl leading-none">{pint.rating.toFixed(1)}</span>
          <span className="text-stout/50 text-xs font-bold ml-0.5">/5</span>
        </div>
      </section>
      ...
      <main className="px-5 py-7">
        ...
        <div className="flex items-center justify-between pt-6 border-t border-cream/5">
          <div>
            ...
            <p className="text-sm font-bold">@{pint.user}</p>
          </div>
